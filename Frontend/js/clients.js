...
const CLIENTS_URL = `${API_BASE_URL}/clients`;
    ...
    function fillEditClientForm(cliente) {
        ...
        setInputValue("editarClienteCodigoArea", cliente.codigo_area || "");
...
const validacionTelefono = validatePhoneData(payload.codigo_area, payload.telefono);

if (!validacionTelefono.ok) {
    form.classList.add("was-validated");
    showAddFormAlert(validacionTelefono.mensaje);
    return;
}

payload.codigo_area = validacionTelefono.codigo_area;
payload.telefono = validacionTelefono.telefono;
    ...
    if (!validacionTelefono.ok) {
        form.classList.add("was-validated");
        showEditFormAlert(validacionTelefono.mensaje);
        return;
    }
    ...
    function buildAddClientPayload() {
        return {
            ...
            codigo_area: getInputValue("clienteCodigoArea"),
    ...
    function buildEditClientPayload() {
        return {
            ...
            codigo_area: getInputValue("editarClienteCodigoArea"),
        ...
        tbody.innerHTML = clientes.map((cliente) => {
            ...
            return `
                <tr>
                    ...
                    <td>${escapeHtml(`+${cliente.codigo_area || ""} ${cliente.telefono || "-"}`)}</td>
...
function validatePhoneData(codigoArea, telefono) {
    const areaLimpia = cleanPhoneNumber(codigoArea);
    const telefonoLimpio = cleanPhoneNumber(telefono);

    if (!areaLimpia) {
        return {
            ok: false,
            mensaje: "Debe ingresar el número de área."
        };
    }

    if (areaLimpia.length < 1 || areaLimpia.length > 4) {
        return {
            ok: false,
            mensaje: "El número de área debe tener entre 1 y 4 dígitos."
        };
    }
    ...
    if (!telefonoLimpio) {
        return {
            ok: false,
            mensaje: "Debe ingresar el número de teléfono."
        };
    }
